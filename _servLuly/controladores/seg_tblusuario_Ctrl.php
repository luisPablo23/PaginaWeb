<?php
//gen 25/2/2024 22:22:14 dst
class seg_tblusuario_Ctrl
{
...
$Con = $oDav_Ctrol->fnDevuelveConsulta('seg_visusuario' , $pCampo0  , $pValor0  , $pCampo1  , $pValor1  , $pCampo2  , $pValor2  , $pCampo3  , $pValor3  , $pCampo4  , $pValor4  , $pCampo5  , $pValor5  , $pCampo6  , $pValor6  , $pCampo7  , $pValor7 );
$sql = $Con;
try {
...
public function papusuario($f3)
{
$idusuario = is_null($f3->get('POST.idusuario')) ? 'T' : $f3->get('POST.idusuario');
$llave = is_null($f3->get('POST.llave')) ? 'T' : $f3->get('POST.llave');
if ($this->M_Usuariologueado->ValidaSession($idusuario, $llave,'papusuario',$f3)) {
...
$idsucursal = is_null($f3->get('POST.pidsucursal')) ? 'T' : $f3->get('POST.pidsucursal');
$idrol = is_null($f3->get('POST.pidrol')) ? 'T' : $f3->get('POST.pidrol');
$sql = "CALL " . $procedure . "('" . $Tipo . "','". $idusuario . "','". $idpersona . "','". $nombreusuario . "','". $clave . "','". $salt . "','". $pin . "','". $idestadousuario . "','". $idtipousuario . "','". $estilo . "','". $idsucursal . "','". $idrol. "'); "; 
try {
$resulto = $f3->get('DB')->exec($sql);
$items = array();
$msg = '200';
